...
import { Send, Mail, Phone, User, MapPin, CheckCircle } from "lucide-react";
    ...
    <section className="bg-[#00020f] min-h-screen text-white font-sans selection:bg-red-500 selection:text-white pt-24 pb-20 px-4 md:px-8">
          ...
          <div className="rounded-3xl border border-slate-800 bg-[#0b1120] p-8">
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <button type="submit" className="flex w-full items-center justify-center gap-2 rounded-xl bg-red-600 py-4 font-semibold text-white transition hover:bg-red-700"> <Send size={18} /> Send Message </button>
              </form>
            )}
          </div>

          <div className="rounded-3xl bg-white p-10">
            <span className="rounded-full bg-red-100 px-4 py-2 text-xs font-semibold uppercase tracking-[0.2em] text-red-600"> Contact Information </span>
            <h2 className="mt-6 text-3xl font-black text-slate-900"> Reach Our Editorial Team </h2>
            <p className="mt-4 leading-8 text-slate-600"> For news tips, corrections, partnerships, interviews, or general inquiries, you can contact us directly using the information below.</p>
            <div className="mt-10 space-y-8">
              <div className="flex items-start gap-5">
                <div className="rounded-2xl bg-red-100 p-4 text-red-600"> <User /> </div>

                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500"> Editor-in-Chief </p>
                  <h3 className="mt-1 text-xl font-bold text-slate-900"> John Ferdinand </h3>
                </div>
              </div>

              <div className="flex items-start gap-5">
                <div className="rounded-2xl bg-indigo-100 p-4 text-indigo-600"> <Mail /> </div>

                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500"> Email </p>
                  <h3 className="mt-1 text-lg font-semibold text-slate-900"> editor@yournews.com </h3>
                </div>
              </div>

              <div className="flex items-start gap-5">
                <div className="rounded-2xl bg-green-100 p-4 text-green-600"> <Phone /> </div>

                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500"> Phone </p>
                  <h3 className="mt-1 text-lg font-semibold text-slate-900"> +234 800 000 0000 </h3>
                </div>
              </div>

              <div className="flex items-start gap-5">
                <div className="rounded-2xl bg-amber-100 p-4 text-amber-600"> <MapPin /> </div>

                <div>
                  <p className="text-xs font-semibold uppercase tracking-[0.2em] text-slate-500"> Office </p>
                  <h3 className="mt-1 text-lg font-semibold text-slate-900"> Warri, Delta State, Nigeria </h3>
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="mt-24 overflow-hidden rounded-[32px] bg-gradient-to-r from-red-600 via-red-700 to-red-800">
        <div className="px-8 py-16 text-center md:px-16">
          <span className="inline-flex rounded-full border border-white/20 bg-white/10 px-4 py-2 text-xs font-semibold uppercase tracking-[0.25em] text-white"> Community Journalism </span>
          <h2 className="mt-6 text-4xl font-black text-white md:text-5xl"> Have A Story That Matters? </h2>
          <p className="mx-auto mt-6 max-w-3xl text-lg leading-8 text-red-100"> If you've witnessed an important event, have verified information, noticed an error in one of our reports, or have a story that deserves public attention, we'd like to hear from you. </p>

          <div className="mt-10 flex flex-wrap justify-center gap-4">
            <button onClick={() => window.scrollTo({top: 0, behavior: "smooth"})} className="rounded-xl bg-white px-8 py-4 font-semibold text-red-700 transition hover:bg-slate-100"> Submit a News Time </button>
            <a href="mailto:editor@yournews.com" className="rounded-xl border border-white/30 px-8 py-4 font-semibold text-white transition hover:bg-white hover:text-red-700"> Email the Editor </a>
          </div>
        </div>
      </section>
    </section>
