      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-xl p-6 sm:p-8 border border-slate-100 transition-all">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-sm text-slate-500 max-w-xs mx-auto">
            Create a secure, complex password to finish restoring access to your admin dashboard.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5" noValidate>
          ...
          <div className="space-y-1.5">
            <label htmlFor="password" className="block text-xs font-semibold text-slate-700 tracking-wider uppercase">
              New Password
            </label>
          ...
          <div className="space-y-1.5">
            <label htmlFor="confirmPassword" className="block text-xs font-semibold text-slate-700 tracking-wider uppercase">
          ...
          <div className="bg-slate-50 rounded-xl p-4 border border-slate-100 space-y-2">
            <p className="text-xs font-bold text-slate-600 uppercase tracking-wide mb-1">Password Requirements</p>
            
            <div className="flex items-center gap-2 text-xs font-medium">
              <span className={`transition-colors ${criteria.minLength ? "text-green-500" : "text-slate-400"}`}>
                {criteria.minLength ? "✓" : "○"} At least 8 characters length
              </span>
            </div>
            
            <div className="flex items-center gap-2 text-xs font-medium">
              <span className={`transition-colors ${criteria.hasNumber ? "text-green-500" : "text-slate-400"}`}>
                {criteria.hasNumber ? "✓" : "○"} At least one numeric digit (0-9)
              </span>
            </div>
            
            <div className="flex items-center gap-2 text-xs font-medium">
              <span className={`transition-colors ${criteria.hasSymbol ? "text-green-500" : "text-slate-400"}`}>
                {criteria.hasSymbol ? "✓" : "○"} At least one special symbol character
              </span>
            </div>

            <div className="flex items-center gap-2 text-xs font-medium pt-1.5 border-t border-slate-200/60">
              <span className={`transition-colors ${criteria.matches ? "text-green-500" : "text-slate-400"}`}>
