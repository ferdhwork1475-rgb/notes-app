...
const AdminProfile = () => {
  ...
  const [profile, setProfile] = useState({
    ...
    email: user?.email || "",
    joinedDate: user?.createdAt ? new Date(user.createdAt).toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) : "Year 2026",
  ...
  const [previewImage, setPreviewImage] = useState(`${uploadPath}${user?.profileImage}` || null);
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-200/60 pb-6">
          <div>
            ...
            <p className="text-sm font-medium text-slate-500 mt-1">
              Configure profile specifications, security tokens, and interface environments.
            </p>
          </div>
          <div className="text-xs font-semibold text-slate-400 bg-white border border-slate-200 px-4 py-2 rounded-xl w-fit shadow-sm">
            System Identity: <span className="text-[#4F46E5] font-bold">Root Master</span>
          </div>
        </div>
          ...
          <div className="lg:col-span-4 bg-white rounded-2xl border border-slate-100 shadow-sm shadow-indigo-100/40 p-6 flex flex-col items-center text-center transition-all">
            ...
            <div className="relative group cursor-pointer mt-4" onClick={() => fileInputRef.current.click()}>
              <div className="w-28 h-28 rounded-full overflow-hidden border-4 border-indigo-50 bg-slate-50 flex items-center justify-center transition-all group-hover:scale-[1.02] shadow-inner relative">
                {previewImage ? (
                  <img src={previewImage ? previewImage : `${uploadPath}${previewImage}`} alt="Avatar View" className="w-full h-full object-cover" />
                ) : (
                  <div className="text-[#4F46E5] font-extrabold text-3xl tracking-tighter">
                    {profile.fullName ? profile.fullName.split(" ").map(n => n[0]).join("").toUpperCase() : "A"}
                  </div>
                )}
              </div>
            ...
            <div className="w-full border-t border-slate-100 my-5 pt-4 text-[11px] text-left text-slate-400 space-y-2">
              ...
              <p className="flex items-center justify-between">
                ...
                <span className="text-slate-600 font-semibold">{profile.joinedDate}</span>
              </p>
            </div>
            ...
            <div className="w-full space-y-1.5 pt-2 border-t border-slate-100">
              ...
              >
                <FileText className={`w-4 h-4 ${activeTab === "details" ? "text-[#4F46E5]" : "text-slate-400"}`} />
                Personal Details
              </button>
              <button
                type="button"
              ...
              >
                <KeyRound className={`w-4 h-4 ${activeTab === "security" ? "text-[#4F46E5]" : "text-slate-400"}`} />
                Security Management
              </button>
            </div>
          </div>
            ...
            {activeTab === "details" ? (
              <form onSubmit={handleSaveDetails} className="p-6 md:p-8 space-y-6" noValidate>
                <div className="border-b border-slate-100 pb-4">
                  <h2 className="text-lg font-extrabold text-[#00020f] tracking-tight">Identity Parameters</h2>
                  <p className="text-xs text-slate-400 mt-0.5">Modify baseline records mapping to your administrative credentials.</p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  <div className="group flex flex-col">
                    ...
                    <div className="relative flex items-center">
                      <User className={`w-4 h-4 absolute left-4 transition-colors ${errors.fullName ? "text-red-400" : "text-slate-400 group-focus-within:text-[#4F46E5]"}`} />
                      <input
                        type="text"
                        name="fullName"
                  ...
                  <div className="group flex flex-col">
                    ...
                    <div className="relative flex items-center">
                      <Mail className={`w-4 h-4 absolute left-4 transition-colors ${errors.email ? "text-red-400" : "text-slate-400 group-focus-within:text-[#4F46E5]"}`} />
                      <input
                        type="email"
                        name="email"
                        value={profile.email}
              ...
              <form onSubmit={handleUpdatePassword} className="p-6 md:p-8 space-y-6" noValidate>
                <div className="border-b border-slate-100 pb-4">
                  <h2 className="text-lg font-extrabold text-[#00020f] tracking-tight">Security Access Control</h2>
                  <p className="text-xs text-slate-400 mt-0.5">Regularly switch cryptography string keys to isolate cluster privileges.</p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  ...
                  <div className="group flex flex-col">
                    ...
                    <div className="relative flex items-center">
                      <Lock className={`w-4 h-4 absolute left-4 transition-colors ${errors.newPassword ? "text-red-400" : "text-slate-400 group-focus-within:text-[#4F46E5]"}`} />
                      <input
                    ...
                    {errors.newPassword && (
                      <p className="text-xs text-red-500 font-semibold flex items-center gap-1 mt-2">
                        <AlertCircle className="w-3.5 h-3.5 flex-shrink-0" /> {errors.newPassword}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="group flex flex-col">
                    ...
                    <div className="relative flex items-center">
                      <Lock className={`w-4 h-4 absolute left-4 transition-colors ${errors.confirmPassword ? "text-red-400" : "text-slate-400 group-focus-within:text-[#4F46E5]"}`} />
                      <input
